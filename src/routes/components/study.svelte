              ...
              <div class="star-rating">
                {#each [1, 2, 3, 4, 5] as rating}
                  <button
                    ...
                    on:mouseenter={() => handleStarHover(rating)}
                    on:mouseleave={handleStarLeave}
                  >
                    <Star
                      size={24}
                      class="star-icon {rating <= effectivenessRating ? 'filled' : ''} {rating <= hoverRating && hoverRating > 0 ? 'highlighted' : ''}"
                    />
                  </button>
                {/each}
              </div>
                  ...
                  <div class="session-details">
                    <div class="session-rating">
                      {#each [1, 2, 3, 4, 5] as rating}
                        <Star
                          size={16}
                          class="star-icon small {rating <= session.rating ? 'filled' : ''}"
                        />
                      {/each}
                    </div>
